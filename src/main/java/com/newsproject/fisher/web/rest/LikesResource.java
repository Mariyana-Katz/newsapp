...
import com.newsproject.fisher.repository.LikesRepository;
...
/**
 * REST controller for managing {@link com.newsproject.fisher.domain.Likes}.
 */
@RestController
...
public class LikesResource {
    ...
    public ResponseEntity<Likes> createLikes(@RequestBody Likes likes) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/likes/" + likes.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, likes.getId().toString()))
            .body(likes);
    }
        ...
        Optional<Likes> result = likesRepository
            ...
            .map(existingLikes -> {
                if (likes.getArticleId() != null) {
                    existingLikes.setArticleId(likes.getArticleId());
                }
                if (likes.getUserId() != null) {
                    existingLikes.setUserId(likes.getUserId());
                }
                if (likes.getCommentId() != null) {
                    existingLikes.setCommentId(likes.getCommentId());
                }
                if (likes.getLikeCount() != null) {
                    existingLikes.setLikeCount(likes.getLikeCount());
                }
