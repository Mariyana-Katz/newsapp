...
const Politics = () => {
  ...
  useEffect(() => {
    FetchArticles()
      .then(data => {
        setArticleData(data);
        const firstHeadline = data.find(article => article.category === 'POLITICS');
  ...
  const filteredArticleData = articleData.filter(article => article.category === 'POLITICS');

  return (
    <div>
      <h1 className="category-header">Politics</h1> {/* Add this line */}
      <div>
